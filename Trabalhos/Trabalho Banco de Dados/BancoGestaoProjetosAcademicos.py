...
def delete_value(connect):
    print("\n---SELECIONAR TABELA PARA EXCLUSÃO---")
    # Exclusão de registros
    cursor = connect.cursor()
    for table_name in tables:
        print("Nome: {}".format(table_name))
    try:
        name = input(str("\nDigite o nome da tabela que deseja excluir registros. "))
        for table_name in tables:
            table_description = tables[table_name]
            if table_name == name:
                print("Para criar a tabela: {}, foi utilizado o seguinte código {}".format(table_name,
        ...
        confirmacao = input("Tem certeza que deseja excluir este registro? (s/n): ")
...
def consultar_groq(pergunta):
    ...
    headers = {
        ...
        "Authorization": "Bearer " #AQUI VAI A CHAVE 
    }
